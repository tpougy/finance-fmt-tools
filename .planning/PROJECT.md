...
- ✓ Formato contábil (`Fin 0D/2D/4D/8D`) construído a partir de uma tabela de tokens nomeados (`FormatTokens.cs`) em vez de strings hardcoded inline — v2.1.0 (2026-07-12), facilita ajustes futuros do formato sem mexer na lógica de montagem
- ✓ Instalador (`scripts/install.ps1`) detecta e migra automaticamente uma instalação VBA legada (`FinanceFmtTools.xlam`) antes de instalar a versão C# — desregistra via Excel COM (`AddIns.Installed = $false`) e remove o arquivo, sem ação manual do usuário — v2.1.0 (2026-07-12), testado em Excel real de ponta a ponta (fixture VBA genuína → detecção → remoção → instalação C# → `Connect=True`)
...
- **Codebase atual**: solução C# com 3 projetos (`FinanceFmtTools.Engine` net48+net8.0, `FinanceFmtTools.ComAddin` net48, `FinanceFmtTools.Engine.Tests` net8.0/xUnit), 40 testes passando, 0 Warnings/0 Errors em `dotnet build`. Add-in COM (`Connect.cs`/`AddInHost.cs`) implementa `IDTExtensibility2`+`IRibbonExtensibility`, GUID fixo `881EFDF3-424C-4240-BCA0-714DAC2B9CD7`/ProgId `FinanceFmtTools.Connect`. Versão atual: **2.1.0**. Formato contábil montado por `AccountingFormatBuilder.Build` a partir de `FormatTokens.cs` (constantes nomeadas); token `PadHyphen` removido na v2.1.0 (resta só `OpenParenPad`/`CloseParenPad`).
- **Instalação**: `scripts/install.ps1`/`uninstall.ps1`/`verify-environment.ps1` — registro 100% HKCU, sem admin, sem `regasm`. Desde v2.1.0, `install.ps1` também detecta e migra automaticamente uma instalação VBA legada (`FinanceFmtTools.xlam`) antes de instalar a versão C#. Testado de ponta a ponta em Excel real (ver abaixo).
...
- **Estado do repositório real**: `tpougy/finance-fmt-tools` é um repositório público real. Teve 2 releases VBA (`v1.0.0`/`v1.0.1`) e, em 2026-07-12, quatro releases C#: `v2.0.0` (bug crítico — add-in nunca conectava), `v2.0.1` (correção do marshaling COM), e `v2.1.0` (migração automática de VBA legado no instalador + remoção do token `PadHyphen` do formato contábil + fix de race condition encontrada em teste ao vivo) — **release recomendada atual**. `main` e `archive/vba-legacy` publicados em `origin`.
...
| Tabela de tokens em C# (`FormatTokens.cs`) em vez de config externa YAML/TOML + geração em tempo de build para o formato contábil | Usuário queria facilitar ajustes futuros do formato sem editar C# diretamente; poucos tokens tornam um arquivo C# dedicado suficiente, sem o custo de um parser/target de build novo | ✓ Good — usuário concordou explicitamente após avaliar o trade-off; usado imediatamente para remover o token `PadHyphen` |
| Migração automática de VBA legado no instalador (detectar `.xlam`, desregistrar via `AddIns.Installed=$false`, apagar arquivo) em vez de manter a orientação manual da v2.0.0 | Usuário pediu explicitamente; evita duas abas "Finance Fmt" simultâneas na Ribbon sem exigir passo manual do usuário | ✓ Good — testado de ponta a ponta em Excel real (fixture VBA genuína criada via COM); encontrou e corrigiu, antes do release, uma race condition real (processo EXCEL.EXE não aguardado) |
