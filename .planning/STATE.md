...
last_activity: 2026-07-13 - Corrigido bug real no one-liner irm|iex (BOM UTF-8 quebrava o parse; scripts agora ASCII puro)
progress:
  total_phases: 5
  completed_phases: 5
  total_plans: 14
...
| 260713-ki0 | Corrigir bug real no one-liner `irm\|iex`: remover BOM UTF-8 e tornar scripts/install.ps1, uninstall.ps1, verify-environment.ps1 ASCII puro | 2026-07-13 | eeec7ea | [260713-ki0-corrigir-bug-real-no-one-liner-de-instal](./quick/260713-ki0-corrigir-bug-real-no-one-liner-de-instal/) |
...
- **CRITICAL BUG found+fixed 2026-07-13, NOT YET RELEASED** (260713-ki0): the v2.1.0-published `scripts/install.ps1`/`uninstall.ps1` break the documented one-liner (`irm <url> | iex`) for every user. Root cause: both files carry a leading UTF-8 BOM (added in `a38ce24` to stop Windows PowerShell 5.1 from corrupting accented Portuguese text when read via `-File`). `Invoke-RestMethod` does not strip that BOM; the leftover `U+FEFF` in front of `#Requires -Version 5.1` breaks `Invoke-Expression`'s parse of the following `[CmdletBinding()]`/`param()` block — reproduced live against the real published v2.1.0 asset via WSL2↔Windows interop, using the user's exact reported command and error text. Considered and rejected a band-aid (strip the BOM inside the one-liner via `TrimStart([char]0xFEFF)`) in favor of removing the root cause: `install.ps1`, `uninstall.ps1`, and `verify-environment.ps1` (for consistency) are now 100% ASCII with no BOM at all — ASCII needs no BOM to disambiguate encoding under any code page, so the *original, unmodified* documented one-liner works correctly again on both the `irm|iex` path and the local `-File` path. Verified via `ParseInput`-over-raw-content (simulating exactly what `irm|iex` receives) — 0 parse errors — and via `Invoke-Expression` reaching real business logic (the "Excel aberto?" check). Also empirically confirmed that naively stripping the BOM from the *original* (still-accented) file content is unsafe: a stray em-dash in a string literal gets corrupted into a string-terminating byte under legacy-codepage misdetection when no BOM is present — this is exactly why ASCII transliteration (not just BOM removal) was required.
  - **This fix is committed locally (`eeec7ea`, `60bb960`, `bf7a384`) but not yet pushed.** Until it's pushed, the `irm|iex` one-liner on `main`/the v2.1.0 release asset remains broken for any user following the README. Recommend a v2.1.1 patch release once pushed, following the same `RELEASE.md` runbook as v2.0.1/v2.1.0 — README.md/RELEASE.md/RELEASE_NOTES.md were deliberately left untouched by 260713-ki0 since the documented one-liner text itself doesn't need to change.
